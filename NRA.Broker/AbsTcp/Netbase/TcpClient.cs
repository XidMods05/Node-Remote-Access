...
public class TcpClient : IDisposable
{
    ...
    internal TcpClient(IPAddress address, int port) : this(new IPEndPoint(address, port))
    {
    }
    ...
    /// <param name="address">IP address</param>
    /// <param name="port">Port number</param>
    internal TcpClient(string address, int port) : this(new IPEndPoint(IPAddress.Parse(address), port))
    {
    }
    ...
    internal TcpClient(DnsEndPoint endpoint) : this(endpoint, endpoint.Host, endpoint.Port)
    {
    }
    ...
    internal TcpClient(IPEndPoint endpoint) : this(endpoint, endpoint.Address.ToString(), endpoint.Port)
    {
    }
    ...
    /// </summary>
    internal int OptionReceiveBufferSize { get; set; } = 8192;
    ...
    /// </summary>
    /// <returns>'true' if the client was successfully disconnected, 'false' if the client is already disconnected</returns>
    internal virtual bool DisconnectAsync()
    {
        return Disconnect();
    }
    ...
    /// </summary>
    /// <returns>'true' if the client was successfully reconnected, 'false' if the client is already reconnected</returns>
    internal virtual bool ReconnectAsync()
    {
        if (!DisconnectAsync())
            return false;
